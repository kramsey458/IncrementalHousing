...
Write-Archive (Join-Path $outDir 'IncrementalHousing-preview6.zip') $payload
$sources = [ordered]@{}
...
Write-Archive (Join-Path $outDir 'IncrementalHousing-preview6-source.zip') $sources
Copy-Item -LiteralPath (Join-Path $modDir 'README.md') -Destination (Join-Path $outDir 'IncrementalHousing-preview6-notes.md')
$checksums = foreach ($file in Get-ChildItem -LiteralPath $outDir -Filter 'IncrementalHousing-preview6*.zip' | Sort-Object Name) {
    '{0}  {1}' -f (Get-FileHash -LiteralPath $file.FullName -Algorithm SHA256).Hash.ToLowerInvariant(), $file.Name
}
$checksums | Set-Content -LiteralPath (Join-Path $outDir 'IncrementalHousing-preview6-SHA256SUMS.txt')
$checksums
